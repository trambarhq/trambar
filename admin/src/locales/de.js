...
module.exports = function(localeCode) {
    return {
        ...
        'activity-tooltip-$count': (count) => {
            return (count === 1) ? `1 Geschichte` : `${count} Geschichten`;
        },
        ...
        'project-list-confirm-archive-$count': (count) => {
            var projects = (count === 1) ? 'das ausgewählte Projekt' : `diese ${count} Projekte`;
        ...
        'project-tooltip-$count-others': (count) => {
            return (count === 1) ? `1 anderes` : `${count} andere`;
        },

        'repo-list-cancel': 'Abbrechen',
        'repo-list-confirm-remove-$count': (count) => {
            var repositories = (count === 1) ? `dieses Repository` : `diese ${count} Repositorys`;
        ...
        'repository-tooltip-$count': (count) => {
            return (count === 1) ? `1 Repository` : `${count} Repositorys`;
        },

        'role-list-add': 'Neue Rolle hinzufügen',
        'role-list-cancel': 'Abbrechen',
        'role-list-confirm-disable-$count': (count) => {
            var roles = (count === 1) ? `diese Rolle` : `diese ${count} Rollen`;
        ...
        'server-list-confirm-disable-$count': (count) => {
            var servers = (count === 1) ? `diesen Server` : `diese ${count} Server`;
        ...
        'task-imported-$count-commit-comments-from-$repo': (count, repo) => {
            var comments = (count === 1) ? `1 Commit-Kommentar` : `${count} Commit-Kommentare`;
            var were = (count === 1) ? `wurde` : `wurden`;
            return `${comments} ${were} aus dem Repository “${repo}” importiert`;
        },
        'task-imported-$count-events-from-$repo': (count, repo) => {
            var events = (count === 1) ? `1 Ereignis` : `${count} Ereignisse`;
            var were = (count === 1) ? `wurde` : `wurden`;
            return `${events} ${were} aus dem Repository “${repo}” importiert`;
        },
        'task-imported-$count-issue-comments-from-$repo': (count, repo) => {
            var comments = (count === 1) ? `1 Problemkommentar` : `${count} Problemkommentare`;
            var were = (count === 1) ? `wurde` : `wurden`;
            return `${comments} ${were} aus dem Repository “${repo}” importiert`;
        },
        'task-imported-$count-merge-request-comments-from-$repo': (count, repo) => {
            var comments = (count === 1) ? `1 Merge-Request-Kommentar` : `${count} Merge-Request-Kommentare`;
            var were = (count === 1) ? `wurde` : `wurden`;
            return `${comments} ${were} aus dem Repository “${repo}” importiert`;
        },
        'task-imported-$count-repos': (count) => {
            var repos = (count === 1) ? `1 Repository` : `${count} Repositorys`;
            var were = (count === 1) ? `wurde` : `wurden`;
        ...
        'user-list-confirm-disable-$count': (count) => {
            var accounts = (count === 1) ? `diesen Benutzer` : `diese ${count} Benutzer`;
