...
module.exports = function(localeCode) {
    return {
        ...
        'activity-tooltip-$count': (count) => {
            return (count === 1) ? `1 histoire` : `${count} histoires`;
        },
        ...
        'project-tooltip-$count-others': (count) => {
            return (count === 1) ? `1 autre` : `${count} autres`;
        },
        ...
        'repository-tooltip-$count': (count) => {
            return (count === 1) ? `1 dépôt` : `${count} dépôts`;
        },
        ...
        'task-imported-$count-commit-comments-from-$repo': (count, repo) => {
            var comments = (count === 1) ? `1 commentaires sur un commit` : `${count} commentaires sur un commit`;
            return `Importé ${comments} from “${repo}”`;
        },
        'task-imported-$count-events-from-$repo': (count, repo) => {
            var events = (count === 1) ? `1 événement` : `${count} événements`;
            return `Importé ${events} from “${repo}”`;
        },
        'task-imported-$count-issue-comments-from-$repo': (count, repo) => {
            var comments = (count === 1) ? `1 commentaire sur un problème` : `${count} commentaires sur un problème`;
            return `Importé ${comments} from “${repo}”`;
        },
        'task-imported-$count-merge-request-comments-from-$repo': (count, repo) => {
            var comments = (count === 1) ? `1 commentaire sur une demande de fusion` : `${count} commentaires sur une demande de fusion`;
            return `Importé ${comments} from “${repo}”`;
        },
        'task-imported-$count-repos': (count) => {
            var repos = (count === 1) ? `1 dépôt` : `${count} dépôts`;
            return `Importé ${repos}`;
        },
        'task-imported-$count-users': (count) => {
            var users = (count === 1) ? `1 utilisateur` : `${count} utilisateurs`;
